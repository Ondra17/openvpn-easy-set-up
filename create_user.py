...
def createStruc(username):
    ...
    if os.path.isfile(f"/etc/openvpn/users/{username}"):
        ...
        cmd = f"sudo cp /etc/openvpn/easy-rsa/pki/issued/{username}.crt /etc/openvpn/users/{username}.crt"
        result = subprocess.run(cmd, shell=True, capture_output=True, text=True)
        #os.system(f"sudo cp /etc/openvpn/easy-rsa/pki/issued/{username}.crt /etc/openvpn/users/{username}.crt")

        cmd = f"sudo cp /etc/openvpn/easy-rsa/pki/private/{username}.key /etc/openvpn/users/{username}.key"
        result1 = subprocess.run(cmd, shell=True, capture_output=True, text=True)
        #os.system(f"sudo cp /etc/openvpn/easy-rsa/pki/private/{username}.key /etc/openvpn/users/{username}.key")

        print("User certificate were created successfully")
        cmd = f"cp /etc/openvpn/easy-rsa/pki/ca.crt /etc/openvpn/users/{username}"
        result2 = subprocess.run(cmd, shell=True, capture_output=True, text=True)
        #os.system(f"cp /etc/openvpn/easy-rsa/pki/ca.crt /etc/openvpn/users/{username}")

        cmd = f"cp /etc/openvpn/user.ovpn /etc/openvpn/users/{username}"
        result3 = subprocess.run(cmd, shell=True, capture_output=True, text=True)
        #os.system(f"cp /etc/openvpn/user.ovpn /etc/openvpn/users/{username}")
        
        if result.returncode != 0 and result1.returncode != 0 and result2.returncode != 0 and result3.returncode != 0:
